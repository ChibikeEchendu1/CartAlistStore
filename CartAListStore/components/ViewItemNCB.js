...
class StoreEditOneItemNBCView extends Component {
  ...
  onButtonPress() {
    const {password, type, input5} = this.props;
  ...
  onButtonPress2() {
    this.setState({Error: '', visible: false});
    this.props.DeleteBC(this.state.item);
  }
  ...
  sendOnWhatsApp = async () => {
    if (
      ...
      if (mobile) {
        if (msg) {
          this.props.sendTextNBC(msg, mobile, this.state.item);
        } else {
  ...
  sendEmail = () => {
    ...
    if (expression.test(String(email).toLowerCase())) {
      if (msg) {
        this.props.sendEmailNBC(msg, email, this.state.item);
      } else {
        this.setState({reordererroe: 'Please insert message to send'});
      }
    } else {
      ...
      return (
        <Card>
          ...
          <Input
            ...
            value={this.props.input5}
            onChangeText={this.onInput5C.bind(this)}
            inputStyle={{}}
            errorStyle={{color: 'red'}}
            errorMessage={this.props.EmailError}
            inputContainerStyle={{
